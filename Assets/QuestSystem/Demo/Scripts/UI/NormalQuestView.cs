...
public class NormalQuestView : QuestView
{
    ...
    {
        _isRewarding = true;
        if(_quest.Rewards!=null && _quest.Rewards.Count > 0)
        {
            ...
            while (t < 2)
            {
                ...
                for (int i = 0; i < _quest.Rewards.Count; i++)
                {
                    ...
                    {
                        pos = Vector3.Lerp(original, _rectMoneyGroup.transform.position, t / 2f);
                    }
                    else if (_quest.Rewards[i].rewardItem == RewardItem.Crystal)
                    {
                        pos = Vector3.Lerp(original, _rectCrystalGroup.transform.position, t / 2f);
                    }
